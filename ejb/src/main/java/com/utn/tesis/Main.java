...
public class Main {
    ...
    private Paragraph campoSiNoDraw(Paragraph p, String preg, Boolean sino) throws DocumentException, IOException {
        ...
        p.add(new Chunk(preg + ": ", questionFont));
        p.add(Chunk.NEWLINE);
        Image check = Image.getInstance("D:\\Documentos\\Desktop\\GitHub SAPO\\Odontologia\\ejb\\src\\main\\resources\\HCPDF\\checked_checkbox.png");
    ...
    public void createPDF(File pdfNewFile, Paciente p) {
        if (p == null)
            return;
        ...
        try {
            ...
            try {
                ...
                if (nullOVacio(p.getDomicilio()) || nullOVacio(p.getDomicilio().getCodigoPostal()))
                    par1.add(new Chunk("" + text(p.getDomicilio().getCodigoPostal()), answerFont));
                else
                ...
                throw new RuntimeException("Error al generar Historia Clinica, datos incompletos.");
            }
    ...
    public void crearBonoConsultaPDF(File pdfNewFile) {
        try {
            ...
            try {
                ...
                par.add(new Chunk(Chunk.NEWLINE));
                par.add(new Chunk(Chunk.NEWLINE));
                par.add(new Chunk("Bono Nº: " + Math.round(Math.random()*1000000000), answerFont));
                par.add(new Chunk(Chunk.NEWLINE));
                par.add(new Chunk(Chunk.NEWLINE));
    ...
    public static Paciente crearPacientePrueba() {
        Paciente p = new Paciente();
        HistoriaClinica hc = HistoriaClinica.createDefault();

        p.setNombre("Maximiliano");
        p.setApellido("Barros");
        p.setEmail("maxibarros2016@gmail.com");
        p.setCelular("3512112322");
        Documento doc = new Documento();
        doc.setNumero("34922121");
        doc.setTipoDocumento(TipoDocumento.DNI);
        p.setDocumento(doc);
        Domicilio domicilio = new Domicilio();
        domicilio.setCalle("Jose Patiño de las avellanas");
        domicilio.setCodigoPostal(5000);
        domicilio.setDepartamento("Capital");
        domicilio.setNumero("3122");
        domicilio.setPiso(3);
        Barrio b = new Barrio();
        b.setNombre("Jardines de talleres");
        Ciudad c = new Ciudad();
        c.setNombre("Ciudad de cordoba");
        Provincia pro = new Provincia();
        pro.setNombre("Cordoba");
        c.setProvincia(pro);
        b.setCiudad(c);
        domicilio.setBarrio(b);
        p.setDomicilio(domicilio);
        p.setEstadoCivil(EstadoCivil.SOLTERO);
        p.setLugarDeNacimiento(c);
        p.setMedicoCabecera("Dr. Jose Ramon Diaz");
        p.setNacionalidad(Nacionalidad.ARGENTINO_NATIVO);
        p.setNivelEstudio(NivelEstudio.UNIVERSITARIO_INCOMPLETO);
        ObraSocial os = new ObraSocial();
        os.setNombre("Obra social Pedorra");
        p.setObraSocial(os);
        p.setReligion("Ateo");
        p.setServicioEmergencia("EMI");
        p.setTelefonoMedicoCabecera("3822999212");
        Trabajo t = new Trabajo();
        t.setNombre("Tecnico informatico");
        p.setTrabajo(t);
        p.setFechaCarga(Calendar.getInstance());
        p.setFechaNacimiento(Calendar.getInstance());
        p.setSexo(Sexo.MASCULINO);

        Usuario u = new Usuario();
        u.setNombreUsuario("Nombre de usuario");
        u.setContrasenia("contraseña");
        u.setEmail("mailusuario@hotmail.com");
        RolUsuario rolu = new RolUsuario();
        rolu.setPersona(p);
        ArrayList<RolUsuario> roles = new ArrayList<RolUsuario>();
        roles.add(rolu);
        u.setRoles(roles);
        hc.setId(1l);
        hc.setRealizoHistoriaClinica(u);

        Atencion aten = new Atencion();
        aten.setFechaAtencion(Calendar.getInstance());
        aten.setFechaDeCarga(Calendar.getInstance());
        aten.setMotivoConsultaOdontologica("Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor ");
        aten.setDescripcionProcedimiento("Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. ");
        aten.setDiagnosticoSolucionado(true);
        AsignacionPaciente asig = new AsignacionPaciente();
        Alumno alu = new Alumno();
        alu.setNombre("Ignacio Javier");
        alu.setApellido("Lopez Arzuaga");
        asig.setAlumno(alu);
        Profesor prof = new Profesor();
        prof.setNombre("Enzo Carlos");
        prof.setApellido("Biancato");
        asig.setAutorizadoPor(prof);
        asig.setPaciente(p);

        TrabajoPractico tp = new TrabajoPractico();
        tp.setDescripcion(" Ipsum es simplemente el texto de relleno de las imprentas y ");
        tp.setNombre("Nombre Trab Pract");
        PracticaOdontologica po = new PracticaOdontologica();
        po.setDenominacion("Nombre pract Odont");
        tp.setPracticaOdontologica(po);

        Diagnostico diag = new Diagnostico();
        diag.setId(1l);
        diag.setObservaciones("Observaciones de diagnostico  Ipsum es simplemente el texto de relleno de las imprentas y ");
        diag.setFechaCreacion(Calendar.getInstance());
        diag.setPracticaNoExistente("Practica odontologica no existente");
        diag.setPracticaOdontologica(po);
        MovimientoDiagnostico mov = new MovimientoDiagnostico();
        mov.setFechaMovimiento(Calendar.getInstance());
        mov.setObservaciones("Observaciones del movimiento diagnostico dle paciente bla bla bla");
        mov.setEstado(EstadoDiagnostico.PENDIENTE);
        diag.setUltimoMovimiento(mov);
        asig.setDiagnostico(diag);

        aten.setAsignacionPaciente(asig);
        hc.addDiagnostico(diag);
        hc.addAtencion(aten);
        hc.addDiagnostico(diag);
        hc.addAtencion(aten);
        hc.addDiagnostico(diag);
    ...
    public static void main(String[] args) {
        ...
        Main generatePDFFileIText = new Main();
        //generatePDFFileIText.createPDF(new File("D:/GeneratePDFFileITexta.pdf"), crearPacientePrueba());
        //generatePDFFileIText.crearListaAsignaciones(new File("D:/Generate.pdf"), new ArrayList<AsignacionPaciente>());
